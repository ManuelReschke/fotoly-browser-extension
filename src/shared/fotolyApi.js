...
export class FotolyApiError extends Error {
  constructor(message, { status = 0, payload = null } = {}) {
    super(message);
    this.name = "FotolyApiError";
    this.status = status;
...
export async function uploadFotolyFile(file, settings, onStatus = () => {}) {
  assertReadyForUpload(settings);
  ...
  if (!imageUuid) {
    throw new FotolyApiError("The upload response did not contain an image ID.");
  }
...
async function uploadToSession(file, session, baseUrl) {
  if (!session.upload_url || !session.token) {
    throw new FotolyApiError("The upload session is incomplete.");
  }

  const formData = new FormData();
  formData.append("file", file, file.name || "fotoly-upload.png");
...
async function waitForProcessing(imageUuid, settings, onStatus) {
  for (let attempt = 1; attempt <= STATUS_POLL_ATTEMPTS; attempt += 1) {
    ...
    if (status.failed) {
      throw new FotolyApiError("Image processing failed.");
    }
  ...
  throw new FotolyApiError("Image processing is taking too long. Please check again later.");
}
...
async function apiJson(settings, path, options = {}) {
  ...
  return readJsonResponse(response, "API request failed.");
}
...
async function readJsonResponse(response, fallbackMessage) {
  ...
  if (!response.ok) {
    throw new FotolyApiError(apiErrorMessage(response.status, payload, fallbackMessage), {
...
function resolveShareUrl(baseUrl, image, uploadResult) {
  ...
  if (!candidate) {
    throw new FotolyApiError("Fotoly did not return a share link.");
  }
...
function apiErrorMessage(status, payload, fallbackMessage) {
  ...
  if (status === 415) {
    return "Fotoly does not accept this file type.";
  }
  ...
  if (status === 503) {
    return "Fotoly is not accepting uploads right now.";
  }
