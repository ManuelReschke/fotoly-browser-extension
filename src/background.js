...
import { uploadFotolyFile } from "./shared/fotolyApi.js";
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === "FOTOLY_PREPARE_SCREENSHOT") {
  ...
  if (message?.type === "FOTOLY_UPLOAD_PENDING") {
...
async function prepareScreenshotFlow(mode = "selection") {
  ...
  if (!settings.apiKey) {
    throw new Error("Please save a Fotoly API key in settings first.");
  }

  if (!(await hasOriginPermission(settings.baseUrl))) {
    throw new Error("The host permission for this Fotoly base URL is missing. Open settings and save again.");
  }
  ...
  const pendingUpload = await savePendingUpload({
    blob: screenshotBlob,
    filename: `fotoly-screenshot-${timestampSlug()}.png`,
    source
  });
...
async function uploadPendingFlow(pendingUploadId) {
  ...
  const file = new File([pendingUpload.blob], pendingUpload.filename || `fotoly-screenshot-${timestampSlug()}.png`, {
    type: pendingUpload.mimeType || pendingUpload.blob.type || "image/png"
  });

  const result = await uploadFotolyFile(file, settings, (phase, message) => {
