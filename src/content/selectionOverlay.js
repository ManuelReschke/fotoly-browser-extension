(() => {
  if (window.__fotolySelectionOverlayLoaded) {
    return;
  }

  window.__fotolySelectionOverlayLoaded = true;

  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type !== "FOTOLY_SELECT_AREA") {
      return false;
    }
  ...
  function startSelection() {
    return new Promise((resolve) => {
      const existing = document.getElementById("fotoly-selection-root");
      if (existing) {
        existing.remove();
      }

      const root = document.createElement("div");
      root.id = "fotoly-selection-root";

      const hint = document.createElement("div");
      hint.className = "fotoly-selection-hint";
      hint.textContent = "Drag to select an area. Press Esc to cancel.";

      const box = document.createElement("div");
      box.className = "fotoly-selection-box";
