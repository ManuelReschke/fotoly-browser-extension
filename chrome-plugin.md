# Fotoly Browser Extension for Chrome, Brave, and Chromium

## Goal

There should be an official browser extension that lets users quickly upload
images and screenshots to Fotoly. The most important feature is a rectangular
screenshot selection from the currently visible page. After upload, the
extension should show the Fotoly share link and make it easy to copy.

The extension is a dedicated Fotoly client and should preferably live in a
separate repository, for example `fotoly-browser-extension`. The Fotoly
...
- Settings page for:
  - Fotoly API key.
  - Fotoly base URL, defaulting to `https://fotoly.eu`.
...
## Upload Flow Against the Fotoly API
...
9. Fetch the final resource:
   ...
   - If `view_url` is relative, resolve it against the configured Fotoly base
     URL.
...
- Host permissions for `https://fotoly.eu/*`.
...
- After a successful upload, a valid Fotoly share link appears.
...
- The extension supports Fotoly and restricted local development URLs.
